...
const CertificateStore: React.FC = () => {
  ...
  useEffect(() => {
    const storedCertificates = JSON.parse(localStorage.getItem('certificates') || '[]') as CertificateData[];
  ...
  const parseCertificate = (file: File) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        const validity = new Date(certificate.notAfter.value).toLocaleDateString();
        const issuer = certificate.issuer.typesAndValues.find(attr => attr.type === '2.5.4.3')!.value.valueBlock.value;
        ...
        setError("Invalid certificate format. Please select a valid certificate file.");
    ...
    <div className="container">
          ...
          {certificates.map((cert, index) => (
            <tr key={index}>
              <td className="common-name">
                <button className="certificate-button" onClick={() => handleCertificateClick(cert)}>{cert.commonName}</button>
              </td>
      ...
      {selectedCertificate && (
          <div className="details-container">
            ...
            <p><strong>Validity:</strong> {selectedCertificate.validity}</p>
